...
// и свойства фаз, параметры для решения СЛАУ
mfe::mfe()
{
	{
		FILE* file;
...
double mfe::sigma(int field)
{
	return 1;
}

double mfe::rightPart(int field, double r, double t)
{
	/*if (field == 0)
		return 20;
	else
...
void mfe::buildLocalG(int ielem)
{
...
void mfe::buildLocalF(int ielem, double dt, double t, double q0[3])
{
	double r1 = MeshRZ[FE[ielem][0]].first;
	double r2 = MeshRZ[FE[ielem][1]].first;
	double r3 = MeshRZ[FE[ielem][2]].first;
	...
	p_loc[0] = rightPart(ielem, r1, t);
	p_loc[1] = rightPart(ielem, r2, t);
	p_loc[2] = rightPart(ielem, r3, t);
	...
	for (int i = 0; i < 3; i++)
		b_loc[i] = c[0] * p_loc + M[i][0] * q0[0] + M[i][1] * q0[1] + M[i][2] * q0[2];
...
void mfe::make_bc_1(double t)
{
	...
	for (int i = 1; i <= z_coord.size(); i++) {
		...
		bc1[i].second = u_t(r_coord.back(), z_coord[i - 1], t);
...
void mfe::writeToFile(mvector& q, double t)
{
	...
	if (File) {
		...
		for (int i = 0; i < Nuz; i++)
			fprintf(File, "%-20.10f%-20.10f%-20.10f%-20.10f%\n", t, q[i], u_t(r_coord[i], t), abs(q[i] - u_t(r_coord[i], t)));
		fclose(File);
	}
	else
		cout << "File q.txt is not opened!";
}

void mfe::iterationProcess() {

	for (int i = 0; i < Nuz; i++)
		q0[i] = u_t(r_coord[i], time[0]);//U(t0)
